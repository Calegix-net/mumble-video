...
// windows.graphics.capture.h/.interop.h are not used here at all, even though they exist on every
// toolchain this project targets - see WgcCaptureCompat.h's own class comment for why a header that is
// merely present is not the same as one that is usable, and why the fix ended up being "stop depending on
// it" rather than another attempt at coaxing its internal include order into behaving.
#include "WgcCaptureCompat.h"
